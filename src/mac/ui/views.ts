import type { AgentItem, ApiRecord, CommandAuditRecord, DashboardData, DashboardState, HermesStartup, JsonObject, ObservabilityEvent, OperatorNotification, RolloutStatus, TaskDetail, TaskRecord, Tone } from "./models.js";
...
export function renderView(state: DashboardState): string {
...
function renderOverview(state: DashboardState): string {
...
function renderAgents(state: DashboardState): string {
  const data = mustData(state);
...
function renderTasks(state: DashboardState): string {
  const data = mustData(state);
...
function renderHermes(state: DashboardState): string {
  const data = mustData(state);
  return `
    <section class="metric-grid">
...
function renderRuntime(state: DashboardState): string {
  const data = mustData(state);
  return `
    <section class="split">
      <div class="surface">
...
function renderSecrets(state: DashboardState): string {
  const data = mustData(state);
  return `
    <section class="split">
      <div class="surface">
...
function renderObservability(state: DashboardState): string {
  const data = mustData(state);
...
function mustData(state: DashboardState): DashboardData {
