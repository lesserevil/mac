import type { DashboardState, JsonObject } from "./models.js";
...
export function createActionHandler(context: ActionHandlerContext): (event: SubmitEvent) => Promise<void> {
  const { api, state, loadDashboard, render } = context;

  return async function handleActionSubmit(event: SubmitEvent): Promise<void> {
...
async function runAction(api: DashboardApi, action: string, form: HTMLFormElement, values: JsonObject): Promise<unknown> {
  const { postJSON } = api;
